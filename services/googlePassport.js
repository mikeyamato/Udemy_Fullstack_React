...
passport.use(
	...
	// we need to tell passport how to use GoogleStrategy within our app
	// `passport.use()` is the medium that the strategy will use
	// `new GoogleStrategy()` creates a new instance of GoogleStrategy
	// pass "options" (aka keys) to 'GoogleStrategy'
	new GoogleStrategy(
		{
		...
		async (accessToken, refreshToken, profile, done) => {
			...
			if (existingUser) {
				...
				// record, 'existingUser'. 
				// by adding 'return' we can remove 'else'. we still need the 'else'
				// logic
